...
public class SuburbRepositoryImpl implements SuburbRepository, PanacheRepositoryBase<SuburbEntity, Integer> {
    ...
    public List<Suburb> getSuburbsByCity(Integer idCity) {
        TypedQuery<SuburbEntity> query =
                em.createQuery("SELECT s FROM SuburbEntity s WHERE s.idCity.id = :idCity", SuburbEntity.class);
        query.setParameter("idCity", idCity);
        return query.getResultList().stream().map(SuburbMapper::toDomain).collect(Collectors.toList());
    }
}
